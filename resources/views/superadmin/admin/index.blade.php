...
@section('content')
                ...
                <div class="card">
                    ...
                    <div class="card-header border-0 pt-6">
                        ...
                        <div class="card-toolbar">
                            ...
                            @include('superadmin.admin.create')
                            <!--end::Modal - Add task-->
                                ...
                                <tbody class="text-gray-600 fw-semibold">
                                    @foreach ($dataAdmin as $index => $admin)
                                        <tr>
                                            ...
                                            <td>{{ $dataAdmin->firstItem() + $index }}</td>
                                            <td>{{ $admin->nik ?? '-' }}</td>
                                            <td>{{ $admin->nipa ?? '-' }}</td>
                                            <td>{{ $admin->nama ?? '-' }}</td>
                                            {{-- <td>{{ $admin->tempat_lahir ?? '-' }}</td>
                                            <td>{{ $admin->tanggal_lahir ? \Carbon\Carbon::parse($admin->tanggal_lahir)->format('d-m-Y') : '-' }}
                                            </td>
                                            ...
                                            {{-- <td>
                                                @php
                                                    ...
                                                    if (Str::startsWith($foto, 'foto_admin/')) {
                                            ...
                                            <td class="text-end">
                                                ...
                                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                    ...
                                                    <div class="menu-item px-3">
                                                        <a href="#" data-bs-toggle="modal"
                                                            data-bs-target="#modal_edit_admin_{{ $admin->id }}"
                                        ...
                                        @include('superadmin.admin.detail')
                                        <!--end::Modal - Detail task-->
                                        <!--begin::Modal - Edit task-->
                                        @include('superadmin.admin.edit')
                                        <!--end::Modal - Edit task-->
                                        <!--begin::Modal - Delete task-->
                                        @include('superadmin.admin.delete')
                                        <!--end::Modal - Delete task-->
                                    @endforeach
                                    {{-- Baris "Tidak ada data" --}}
                                    <tr id="row-no-data" @if (count($dataAdmin)) style="display: none" @endif>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const searchInput = document.querySelector('[data-admin-table-filter="search"]');
            const table = document.getElementById('tabel_admin');
